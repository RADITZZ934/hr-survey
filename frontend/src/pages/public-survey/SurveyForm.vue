<template>
  <div class="max-w-2xl mx-auto my-8 bg-white rounded-2xl border border-slate-100 shadow-lg p-6 lg:p-8 space-y-8 animate-fade-in">
      ...
      <div v-if="questions.length > 0" class="space-y-2">
        <div class="flex justify-between items-center bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">
          <span class="text-xs font-bold text-slate-700">
            {{ activeSection === 'star' ? 'Bagian 1: Penilaian Bintang' : 'Bagian 2: Pertanyaan Essay' }}
          </span>
          <span class="text-[10px] bg-[#4647AE]/10 text-[#4647AE] font-semibold px-2 py-0.5 rounded-full uppercase">
            {{ activeSection === 'star' ? `${starQuestions.length} Soal` : `${essayQuestions.length} Soal` }}
          </span>
        </div>
        <!-- Star Legend Note -->
        <div v-if="activeSection === 'star'" class="text-[10px] sm:text-[11px] text-slate-400/80 flex items-center justify-between px-2 font-semibold">
          <span>1 = Buruk (Sangat Tidak Setuju)</span>
          <span>5 = Terbaik (Sangat Setuju)</span>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="questions.length > 0" class="space-y-6">
      ...
      <!-- STEP 1: Rating Bintang -->
      <div v-if="activeSection === 'star'" class="space-y-6">
        <div 
          v-for="(q, idx) in starQuestions" 
          :key="q.id" 
        ...
        <div v-if="starQuestions.length === 0" class="py-12 text-center text-slate-400 italic">
          Tidak ada pertanyaan tipe rating bintang. Silakan lanjut ke bagian berikutnya.
        </div>
      </div>

      <!-- STEP 2: Essay -->
      <div v-if="activeSection === 'essay'" class="space-y-6">
        <div 
        ...
        <div v-if="essayQuestions.length === 0" class="py-12 text-center text-slate-400 italic">
          Tidak ada pertanyaan tipe essay.
        </div>
      </div>
    </div>
    ...
    <div class="flex gap-4 pt-4 border-t border-slate-100">
      <!-- Back button for Essay page -->
      <button 
        v-if="activeSection === 'essay' && starQuestions.length > 0"
        @click="activeSection = 'star'"
      ...
      <!-- Next to Essay button -->
      <button 
        v-if="activeSection === 'star' && essayQuestions.length > 0"
        @click="activeSection = 'essay'"
        :disabled="isStarSectionDisabled || loading || questions.length === 0"
...
const activeSection = ref('star'); // 'star' or 'essay'
...
const starQuestions = computed(() => questions.value.filter(q => q.type === 'star'));
...
// Check if there are unanswered required star questions
const isStarSectionDisabled = computed(() => {
  return starQuestions.value.some(q => {
...
onMounted(async () => {
  ...
  if (surveyId) {
    try {
      ...
      // If there are no star questions but there are essay questions, start with essay section
      if (starQuestions.value.length === 0 && essayQuestions.value.length > 0) {
        activeSection.value = 'essay';
      }
    } catch (error) {
